...
class ImageGenerator:
    ...
    def create_animation(self, images, steps):
        ...
        def animate(i):
            ax.clear()
            ax.imshow(images[i])
            ax.set_title(f"Step: {steps - i * interval}")
            ax.axis('off')
        
        anim = FuncAnimation(fig, animate, frames=len(images), interval=200, repeat_delay=1000)
        return anim

    def create_image_grid(self,  images,  grid_size=(5, 5)):        
        rows, cols = grid_size
...
# Create an animation
images = generator.generate_image((1, 3, 32, 32), interval = interval, steps = steps, starting_noise=starting_noise) 
# anim = generator.create_animation(
...
fig = generator.create_image_grid(images = images)
